...
What this suite fixes as CONTRACT (must stay true after unification):
  A. Swing detection is IDENTICAL between both engines (same algorithm copy).
  B. Event-driven invalidation (cross of broken level) is IDENTICAL.
  C. Canonical BOS requires `confirm_bars=2` consecutive body closes (filters
     fakeouts); legacy detectors.bos fires on a single bar. This divergence is
     a DESIGN difference (improvement expected after unification), not a bug.
  D. CHOCH semantics differ by design: detectors uses MAs 20/50 + swing(20);
     canonical breaks the LAST BOS level opposite to its direction. Different
     by decision; after unification detectors must adopt the canonical rule.

Synthetic deterministic data only (no MT5, no network) -> fast + reproducible.

NOTE: `test_post_unification_equivalence` is xfail until the unification commit
lands; after that commit, remove the xfail and require zero divergence.
...
# ---------------------------------------------------------------------------
# B. Event-driven invalidation identical (canonical)
# ---------------------------------------------------------------------------
def test_invalidation_identical_canonical():
    fr = _flat(40, 100.0)
    _set(fr, 10, high=105.0, close=105.0)   # swing high at 105 (exposed from bar 15)
    _set(fr, 20, high=106.0, low=105.5, close=106.0)  # 1 bar break (confirm_bars=1 -> active)
    _hold_above(fr, 21, 32)
    _cross_down(fr, 32, 105.0, 1.0)         # cross back below -> invalidation

    # confirm_bars=1 isolates the INVALIDATION contract (both engines activate on 1 bar)
...
# ---------------------------------------------------------------------------
# C. Canonical requires confirm_bars=2; legacy detectors fires on 1 bar
# ---------------------------------------------------------------------------
def test_canonical_requires_2_confirm_bars():
    fr = _flat(40, 100.0)
    _set(fr, 10, high=105.0, close=105.0)
    _set(fr, 20, high=106.0, low=105.5, close=106.0)  # ONLY ONE bar breaks -> fakeout

    ms = detect_market_structure(fr, StructureConfig(confirm_bars=2))
    assert int((ms["bos_dir"] != 0).sum()) == 0


def test_legacy_detectors_fires_on_1_bar():
    fr = _flat(40, 100.0)
    _set(fr, 10, high=105.0, close=105.0)
    _set(fr, 20, high=106.0, low=105.5, close=106.0)

    det = detect_bos(fr, BosConfig(swing_lookback=5))
    assert int((det["bos_direction"] != 0).sum()) >= 1
    # DOCUMENTED CONTRACT: after unification detectors must adopt confirm_bars=2
    # and this test must be removed/updated (detectors will then require 2 bars).


# ---------------------------------------------------------------------------
# D. CHOCH semantics differ by design (documented, not a regression)
# ---------------------------------------------------------------------------
def test_choch_legacy_vs_canonical_differ_by_design():
    fr = _flat(70, 100.0)
    _set(fr, 10, high=105.0, close=105.0)
    ...
    assert "choch_signal" in det.columns
    assert "choch_dir" in ms.columns
    # Allowed to disagree on direction counts by design (resolved on unify).


# ---------------------------------------------------------------------------
# POST-UNIFICATION equivalence gate (xfail until unification commit)
# ---------------------------------------------------------------------------
@pytest.mark.xfail(reason="Activate after unification commit: detectors must delegate to canonical")
