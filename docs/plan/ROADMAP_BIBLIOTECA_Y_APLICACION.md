...
- [x] **Fase 5 (Medidor de alineación / cierre brechas B/C/A1/E) ✅ CÓDIGO AISLADO 2026-07-19:** los 4 módulos puros TDD verdes (19 tests) cierran las brechas como CALIFICADOR (bonus, no filtro duro): **B** `ict_backtest/poi_anchor.py` (ancla narrativa HTF padre, bonus +0.5); **C** `ict_backtest/dealing_range.py` (premium/discount/EQ, bonus +0.5 si zona correcta); **A1** `ict_backtest/plan_attach.py` (`attach_alignment` adjunta AlignmentReport multi-TF D1/H4/H1/M15/M5/M1, modo OBSERVE); **E** `po3_complete` en `AlignmentReport` (bonus +0.5 si PO3 A/M/D alineado). `score_plan` suma base 4 + ancla + zona + po3 (max ~5.5). Auditoría `docs/plan/AUDITORIA_TESIS_FASE5.md` confirma: el 0 de señales del motor canónico es versión simplificada (no "sin edge"). **Pendiente cablear en `run_backtest.py` (`--attach-plan`, modo observe) + backtest comparativo Score vs WR.** Por tu regla de no sobrefiltrado: el plan MIDE, no descarta. | 🟢 Andando (aislado) | Alta |
