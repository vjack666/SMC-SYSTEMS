...
|| **Breaker Block / MMXM** (zona de entry alternativa al FVG) | El robot solo entra en FVG; ICT usa breaker como falla de OB que se vuelve resistencia. MMXM es el "mapa" del ciclo. | 🟢 Detector en `ict_backtest/setups/breaker_block.py` + **wiring a `canonical.py` (flag_breaker_block, post-señal, no gate) REPARADO 2026-07-22**: `is_breaker_block` no propagaba `start_idx` del OB → call-site real devolvía `breaker_active=False` SIEMPRE (no-op silencioso en vivo, enmascarado por try/except); `_ob_dicts_from_frame` hacía `to_numpy(bool)` sobre NaN → marcaba las 25 velas como OB. Ambos corregidos; 14/14 tests verdes; call-site real verificado sobre frame. | Libro `22_BREAKER_MMXM.md` + detector `ict_backtest/setups/breaker_block.py` |
...
- [x] `ict_backtest/setups/breaker_block.py`: detector + `flag_breaker_block` **CABLEADO y REPARADO 2026-07-22** (bug start_idx + NaN→True corregidos; 14/14 tests verdes; call-site real verificado sobre frame en `canonical.evaluate_signals`).
- [x] `ict_backtest/setups/ote.py`: `flag_ote` **CABLEADO** como flag post-señal en `canonical.evaluate_signals` (no gate duro).
...
- [x] **E1 Trade Management → simulador ✅ MÓDULO CERRADO (2026-07-20) + CABLEADO (Fase 1.2, 2026-07-22):** `ict_backtest/trade_mgmt.py` con `to_breakeven`/`partial_exit`/`trailing_stop` (funciones puras) + `apply_trade_management(entry, sl, tp, direction, df, ...)`. El call-site real en el simulador NO existía hasta Fase 1.2: `simulate_trade` (engine.py) ganó `trade_mgmt: bool = False` y re-simula vía `apply_trade_management` cuando True (mapea exit_reason a TP/SL/BE/TRAILING/hold_limit). Propagado a `simulate_trade_with_context` → `run_sequence_backtest`/`run` + CLI `--trade-mgmt`. Regresión cero (`trade_mgmt=False` = SL/TP simples históricos). Tests `tests/test_e1_trade_mgmt.py` (19) + `tests/test_e1_applied_trade_mgmt.py` (3) + `tests/test_fase1_2_trade_mgmt_wiring.py` (6). Ver `CRONOGRAMA_Y_ROADMAP.md` fila E1/Fase 1.2 + DEC-009l (corrección).
