...
- [x] **Fase 5 (Medidor de alineación / medir brechas B/C/A1/E) ✅ MEDIDOR REPARADO Y VERIFICADO 2026-07-20:** los 4 módulos puros TDD verdes califican (bonus, no filtro duro): **B** `ict_backtest/poi_anchor.py` (ancla narrativa HTF padre **por bar_time**, bonus +0.5); **C** `ict_backtest/dealing_range.py` (premium/discount/EQ, bonus +0.5 si zona correcta); **A1** `ict_backtest/plan_attach.py` (`attach_alignment`) **CABLEADO DE VERDAD en `run_backtest.py`** detrás de `--attach-plan` (modo OBSERVE: adjunta AlignmentReport multi-TF por señal con score REAL, NO filtra, NO cambia PnL). **E** `po3_complete` en `AlignmentReport` (bonus +0.5). `score_plan` suma base 4 + ancla + zona + po3 (max ~5.5). **REPARACIÓN del call site muerto (2026-07-20):** el cableado original (2026-07-19) daba score SIEMPRE 0 porque `build_context_stack` devuelve dicts planos (no MarketObjects) y `emit_m15` esperaba `phase_log` inexistente en `ICTSignal`. Ahora `run_backtest._build_objs_by_tf` construye MarketObjects REALES vía `data_feed.build_objects`, `emit_m15` infiere desde `entry_at`/`bos_at`, y anti-look-ahead usa `bar_time` (no `bar_index` cross-TF). Demo `scripts/fase5_demo_cableado_real.py` PASS (score 5.5). Tests `tests/test_plan_cableado_real.py` + `tests/test_plan_cableado_call_site.py` auditan que el call site NO queda muerto. **ACLARACIÓN DE ALCANCE:** esto repara el MEDIDOR (califica por señal). NO aplica las brechas en el MOTOR BASE: `htf_poi_fn` sigue SIN pasarse en `canonical.py` (Brecha B en motor OFF), D1/H1 no deciden (Brecha A1 en motor), `dealing_range`/`po3` no están en `run_sequence` (Brechas C/E en motor). El medidor VE y CALIFICA, el motor no USA. Auditoría `docs/plan/AUDITORIA_TESIS_FASE5.md` §2-§7. **Pendiente:** (a) backtest comparativo `--attach-plan` para medir Score vs WR y decidir umbral SOLO si hay separación; (b) cablear C/A1/E AL MOTOR BASE (Brecha B YA CERRADA 2026-07-20 vía Opción 2: `ict_backtest/poi_anchor_motor.py` + `ICTSignal.htf_anchored` anotado en postproceso en `canonical.py`, `run_sequence` intacto, no filtra — ver `AUDITORIA_TESIS_FASE5.md` §3; pendiente ejercitar sobre señal real: motor base produjo 0 señales en la verificación). Por tu regla de no sobrefiltrado: el plan MIDE, no filtra duro. | 🟢 Medidor andando / Motor base pendiente | Alta |
