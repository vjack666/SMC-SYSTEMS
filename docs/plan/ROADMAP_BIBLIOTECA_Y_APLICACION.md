...
- [x] **Fase 5 (Medidor de alineación / cierre brechas B/C/A1/E) ✅ CÓDIGO + CABLEADO 2026-07-19:** los 4 módulos puros TDD verdes (21 tests, incluye test de integración `run_backtest --attach-plan`) cierran las brechas como CALIFICADOR (bonus, no filtro duro): **B** `ict_backtest/poi_anchor.py` (ancla narrativa HTF padre, bonus +0.5); **C** `ict_backtest/dealing_range.py` (premium/discount/EQ, bonus +0.5 si zona correcta); **A1** `ict_backtest/plan_attach.py` (`attach_alignment`) **CABLEADO en `run_backtest.py`** detrás de `--attach-plan` (modo OBSERVE: adjunta AlignmentReport multi-TF por señal, NO filtra, NO cambia PnL, reusa el market_stack de Fase D); **E** `po3_complete` en `AlignmentReport` (bonus +0.5 si PO3 A/M/D alineado). `score_plan` suma base 4 + ancla + zona + po3 (max ~5.5). Auditoría `docs/plan/AUDITORIA_TESIS_FASE5.md`. **Pendiente:** backtest comparativo `--attach-plan` para medir distribución Score vs WR y decidir umbral SOLO si los datos muestran separación. Por tu regla de no sobrefiltrado: el plan MIDE, no descarta. | 🟢 Andando | Alta |
