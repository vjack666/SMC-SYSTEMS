...
| **Fase 5 (Medidor alineacion / cierre brechas tesis)** | **Cerrar brechas B/C/A1/E de la tesis ICT como CALIFICADOR (no filtro)** | 🟢 ANDANDO (2026-07-19): módulos puros + TDD verdes (21 tests), cableado en producción. **Brecha B (POI anclado):** `ict_backtest/poi_anchor.py` + bonus +0.5 en `score_plan`. **Brecha C (dealing range):** `ict_backtest/dealing_range.py` + bonus +0.5. **Brecha A1 (loop driver):** `ict_backtest/plan_attach.py` (`attach_alignment`) **CABLEADO en `run_backtest.py`** detrás de flag `--attach-plan` (modo OBSERVE: adjunta AlignmentReport multi-TF por señal, NO filtra, NO cambia PnL; reusa el `market_stack` de Fase D). **Brecha E (PO3):** `po3_complete` en `AlignmentReport` + bonus +0.5. `score_plan` suma (max ~5.5): base 4 + ancla + zona + po3. Demos sintéticas en `scripts/fase5_demo_*.py`. **Auditoría:** `docs/plan/AUDITORIA_TESIS_FASE5.md`. **Pendiente:** backtest comparativo `--attach-plan` sobre datos reales para medir distribución de scores (Score vs WR) y decidir umbral SOLO si los datos muestran separación. | 🟢 Andando | Alta |
