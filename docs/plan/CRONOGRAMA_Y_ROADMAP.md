...
| **Fase 5 (Medidor alineacion / cierre brechas tesis)** | **Cerrar brechas B/C/A1/E de la tesis ICT como CALIFICADOR (no filtro)** | 🟢 ANDANDO (2026-07-19): módulos puros + TDD verdes (19 tests), sin tocar producción. **Brecha B (POI anclado):** `ict_backtest/poi_anchor.py` (`anchor_objects`, marca ancla narrativa HTF padre, bonus no filtro) + bonus +0.5 en `score_plan`. **Brecha C (dealing range):** `ict_backtest/dealing_range.py` (`classify_zone` premium/discount/EQ) + bonus +0.5 si zona correcta. **Brecha A1 (loop driver):** `ict_backtest/plan_attach.py` (`attach_alignment`, adjunta AlignmentReport multi-TF D1/H4/H1/M15/M5/M1 a cada señal, modo OBSERVE: NO filtra). **Brecha E (PO3):** `po3_complete` en `AlignmentReport` + bonus +0.5 si PO3 A/M/D alineado. `score_plan` suma (max ~5.5): base 4 + ancla 0.5 + zona 0.5 + po3 0.5. Demos sintéticas en `scripts/fase5_demo_*.py`. **Auditoría de tesis:** `docs/plan/AUDITORIA_TESIS_FASE5.md` (por qué el motor canónico da 0 señales = versión simplificada, no "sin edge"). **Pendiente:** cablear `attach_alignment` en `run_backtest.py` detrás de flag `--attach-plan` (modo observe, no cambia conteo de trades) + backtest comparativo para medir distribución de scores. | 🟢 Andando (aislado) | Alta |
