...
| **Fase 5 (Medidor alineacion / cierre brechas tesis)** | **Medir brechas B/C/A1/E como CALIFICADOR (no filtro) — medidor cableado y verificado** | 🟢 MEDIDOR REPARADO Y VERIFICADO (2026-07-20): módulos puros + TDD verdes + **call site real verificado**. El cableado original (2026-07-19) estaba MUERTO: `build_context_stack` devuelve dicts planos (no MarketObjects), `_stack_to_objs_by_tf` siempre daba `{}` y `emit_m15` esperaba `phase_log` (que ICTSignal no trae) → score SIEMPRE 0. Reparado: (1) `run_backtest._build_objs_by_tf` construye MarketObjects REALES vía `data_feed.build_objects` (Fase A/B/C, sellados por origin_tf+bar_time), una vez por backtest; (2) `emit_m15` infiere STRUCTURE_OK/SETUP_LIVE desde `ICTSignal` real (`entry_at`/`bos_at`); (3) `poi_anchor` y `plan_attach` comparan por `bar_time` (anti-look-ahead cross-TF real); (4) swing HTF cerrado desde `ms` para dealing range real. Demo `scripts/fase5_demo_cableado_real.py` PASS (score 5.5). Tests `tests/test_plan_cableado_real.py` + `tests/test_plan_cableado_call_site.py` auditan que el call site NO queda muerto. **ACLARACIÓN DE ALCANCE (crítica):** esto repara el MEDIDOR (Fase 5, calificador que MIDE score por señal y lo adjunta a `m['alignments']`). NO aplica ninguna brecha en el MOTOR BASE: el motor sigue siendo la versión de 2 TF (H4→M15) y `htf_poi_fn` sigue SIN pasarse en `canonical.py` (Brecha B en motor OFF), D1/H1 no deciden (Brecha A1 en motor), `dealing_range`/`po3` no están en `run_sequence` (Brechas C/E en motor). El medidor las VE y CALIFICA, pero el motor no las USA. Ver `docs/plan/AUDITORIA_TESIS_FASE5.md` §2-§7 (estado real de cada brecha en el motor). **Pendiente:** (a) backtest comparativo `--attach-plan` para medir distribución Score vs WR y decidir umbral SOLO si los datos muestran separación; (b) cablear A1 (3 capas reales: D1/H1 gobiernan) y FSM (loop driver) AL MOTOR BASE. Brechas B/C/E YA CERRADAS 2026-07-20 (Opción 2, postproceso en canonical.py, run_sequence intacto, anota-no-filtra): `ICTSignal.htf_anchored` / `zone_class` / `po3_complete` anotados; pendiente ejercitar sobre señal real porque el motor base produjo 0 señales en la verificación. Ver `AUDITORIA_TESIS_FASE5.md` §3/§4/§7. | 🟢 Medidor andando / Motor base parcial (B/C/E cerradas) | Alta |
