...
| **D (Fase D, Diagnosis Engine)** | **Infraestructura de autoexplicación post-backtest** | 🔶 DISEÑO APROBADO 2026-07-18: el sistema explica POR QUÉ ganó/perdió cuando el PF no alcanza, SIN optimizar params (no es Optuna). Cadena: TradeContext → Statistics → Pattern Detector → Hypothesis Generator → Evidence Ranking → Final Report. **Paso 1 (Fase C viaja a backtests como METADATA, enable_pd_index=True, sin gate, R1 preservado) EN CURSO** — `run_backtest.py` + `v2/orchestrator` + `v2/strategy_mtf` cableados, 2 tests verdes. Paso 2 (TradeContext en `simulate_trade`), Paso 3 (`ict_backtest/diagnostics/`), Paso 4 (alimentar v2 `explanation.layers["PD"]`) PENDIENTES. Ver `docs/plan/ETAPA_DIAGNOSIS_ENGINE_PLAN.md`. NO pushear sin OK. | 🔶 Diseño ✅ / Paso 1 🟢 / 2-4 🔴 | Alta |
