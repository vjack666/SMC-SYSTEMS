...
| **D (Fase D, Diagnosis Engine)** | **Infraestructura de autoexplicación post-backtest** | 🔶 DISEÑO APROBADO 2026-07-18 (refinado 2026-07-18 con 4 reglas de Ruben): el sistema explica POR QUÉ ganó/perdió cuando el PF no alcanza, SIN optimizar params (no es Optuna). Cadena: TradeContext(congelado)→Statistics→Correlation Engine→Hypothesis Generator→Evidence Ranking→Final Report. **Reglas de hierro (2026-07-18):** (1) `TradeContext` @dataclass(frozen=True) INMUTABLE, 1 vez, nunca post-outcome (anti sesgo de retrospectiva = mismo peligro que look-ahead R4); (2) ids persistentes `trade_id`+`signal_id`+`context_version` (trazabilidad R7 + reconstrucción años después); (3) Correlation Engine SEPARADO de Hypothesis Engine (hipótesis solo tras correlaciones, no de estadísticas crudas); (4) Final Report OBLIGATORIAMENTE incluye apartado "¿qué NO puede concluir?" (evidencia insuficiente → "no lo sé"), anti-sobreajuste. **Paso 1 (Fase C viaja a backtests como METADATA, enable_pd_index=True, sin gate, R1 preservado) EN CURSO** — `run_backtest.py`+`v2/orchestrator`+`v2/strategy_mtf` cableados, 2 tests verdes. Paso 2 (TradeContext en `simulate_trade`, frozen) / Paso 3 (`ict_backtest/diagnostics/`) / Paso 4 (alimentar v2 `explanation.layers["PD"]`) PENDIENTES. Ver `docs/plan/ETAPA_DIAGNOSIS_ENGINE_PLAN.md`. Evaluación arquitectura Ruben: 9.8/10. NO pushear sin OK. | 🔶 Diseño ✅ / Paso 1 🟢 / 2-4 🔴 | Alta |
