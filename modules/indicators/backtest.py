...
from modules.indicators.ml_model import score_frame, train_model
...
def run_backtest(config: BacktestConfig | None = None) -> dict[str, float | int]:
    ...
    for symbol in config.symbols:
        ...
        bullish = (scored["ema_spread_atr"] > 0.0) & (scored["ml_confidence"] >= config.confidence_threshold)
        bearish = (scored["ema_spread_atr"] < 0.0) & (scored["ml_confidence"] >= config.confidence_threshold)
    ...
    if not trades:
        raise RuntimeError("No Indicators trades produced.")
    ...
    (out / "indicators_metrics.json").write_text(json.dumps(metrics, indent=2), encoding="utf-8")
    return metrics
