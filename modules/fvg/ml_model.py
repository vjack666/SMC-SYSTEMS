...
FEATURE_COLUMNS = ["fvg_bullish", "fvg_bearish", "gap_size_atr", "atr_ratio", "body_ratio"]
...
def build_feature_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    data["fvg_bullish"] = data["fvg_bullish"].astype(int)
    data["fvg_bearish"] = data["fvg_bearish"].astype(int)
    prev2_high = data["high"].shift(2)
    prev2_low = data["low"].shift(2)
    bull_gap = (data["low"] - prev2_high).clip(lower=0.0)
    bear_gap = (prev2_low - data["high"]).clip(lower=0.0)
    data["gap_size_atr"] = (bull_gap + bear_gap) / data["atr"].replace(0.0, np.nan)
...
def build_labels(frame: pd.DataFrame) -> pd.Series:
    ...
    direction = data["fvg_bullish"].astype(int) - data["fvg_bearish"].astype(int)
...
def train_model(frame: pd.DataFrame) -> RandomForestClassifier:
    ...
    if len(data) < 200:
        raise ValueError("Not enough rows for FVG model training.")
