...
from modules.fvg.ml_model import score_frame, train_model
...
def run_backtest(config: BacktestConfig | None = None) -> dict[str, float | int]:
    ...
    for symbol in config.symbols:
        ...
        bullish = scored["fvg_bullish"] & (scored["ml_confidence"] >= config.confidence_threshold)
        bearish = scored["fvg_bearish"] & (scored["ml_confidence"] >= config.confidence_threshold)
    ...
    if not trades:
        raise RuntimeError("No FVG trades produced.")
    ...
    (out / "fvg_metrics.json").write_text(json.dumps(metrics, indent=2), encoding="utf-8")
    return metrics
