...
from modules.swing.ml_model import score_frame, train_model
...
def run_backtest(config: BacktestConfig | None = None) -> dict[str, float | int]:
    ...
    for symbol in config.symbols:
        ...
        frame = detect_swings(frame, SwingConfig(left_window=4, right_window=4, min_distance=3))

        split = int(len(frame) * 0.6)
        model = train_model(frame.iloc[:split].copy())
        scored = score_frame(frame.iloc[split:].copy(), model)

        bullish = scored["swing_low"] & (scored["ml_confidence"] >= config.confidence_threshold)
        bearish = scored["swing_high"] & (scored["ml_confidence"] >= config.confidence_threshold)
    ...
    if not trades:
        raise RuntimeError("No Swing trades produced.")
    ...
    (out / "swing_metrics.json").write_text(json.dumps(metrics, indent=2), encoding="utf-8")
    return metrics
