...
FEATURE_COLUMNS = ["fractal_high_int", "fractal_low_int", "atr_ratio", "range_ratio", "body_ratio"]
...
def build_feature_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    data["fractal_high_int"] = data.get("fractal_high", False).astype(int)
    data["fractal_low_int"] = data.get("fractal_low", False).astype(int)
...
def build_labels(frame: pd.DataFrame) -> pd.Series:
    ...
    direction = data["fractal_low_int"] - data["fractal_high_int"]
...
def train_model(frame: pd.DataFrame) -> RandomForestClassifier:
    ...
    if len(data) < 200:
        raise ValueError("Not enough rows for Fractal model training.")
