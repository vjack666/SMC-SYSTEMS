...
from signals import ScalpingConfig, ScalpingSignal, build_scalping_context, summarize_filter_diagnosis
...
def _load_ml_model(path: Path) -> Any | None:
    if not path.exists():
        return None
    try:
        from ml.trainer import load_model as _lm
...
def _predict_quality_probability(model: Any | None, feature_row: dict[str, Any], fallback: float) -> float:
    try:
        from ml.trainer import predict_proba as _pp
